...
function parseWeekdays(value: unknown): CalendarWeekday[] {
...
function parseMoons(value: unknown): CalendarMoon[] {
  return asArray(value)
    .map((entry) => {
      const row = asObject(entry);
      if (!row) return null;
...
export function buildCalendarMonthViewport(
  ...
  const climateAspect = segment?.climateAspect ?? DEFAULT_CLIMATE_ASPECT;
