...
export type { PartyParticipation, PartyParticipationRole } from '@shared/partyParticipation';
...
function normalizeAppearance(raw: unknown): CharacterAppearanceMetadata {
  ...
  return {
    ...
    portraitUrl: normalizeNullableText(obj.portraitUrl),
...
/** Read canonical page IDs from metadata for index link rendering */
export function readCharacterIndexIds(
  metadata: unknown,
  campaignNow?: ChronologyDateParts,
): {
  primaryAffiliationId: string | null;
  ...
  return {
    primaryAffiliationId: resolvePrimaryAffiliationId(
      identity,
      lineage,
      campaignNow,
    ),
