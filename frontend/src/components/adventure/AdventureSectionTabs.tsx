...
interface AdventureSectionTabsProps {
  basePath: string;
  activeSection: AdventureSectionId;
}

export function AdventureSectionTabs({ basePath, activeSection }: AdventureSectionTabsProps) {
  ...
  const activeLabel =
    ADVENTURE_SECTIONS.find((section) => section.id === activeSection)?.label ??
    ADVENTURE_SECTIONS[0]?.label ??
    'Story';

  useEffect(() => {
    if (!menuOpen) return;
    const activeIndex = ADVENTURE_SECTIONS.findIndex((section) => section.id === activeSection);
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === 'ArrowDown') {
        ...
        setHighlightIndex((prev) => Math.min(prev + 1, ADVENTURE_SECTIONS.length - 1));
      } else if (event.key === 'ArrowUp') {
        event.preventDefault();
        setHighlightIndex((prev) => Math.max(prev - 1, 0));
      } else if (event.key === 'Enter' || event.key === ' ') {
        event.preventDefault();
        const section = ADVENTURE_SECTIONS[highlightIndex];
        if (section) {
          setMenuOpen(false);
          triggerRef.current?.focus();
        }
  ...
  }, [menuOpen, highlightIndex]);

  function isSectionActive(sectionId: AdventureSectionId): boolean {
  ...
  return (
    <>
      <div className="relative min-w-0 flex-1 max-md:hidden">
        ...
        >
          {ADVENTURE_SECTIONS.map((section) => (
            <NavLink
              key={section.id}
              to={adventureSectionHref(basePath, section.id)}
              role="tab"
              aria-selected={isSectionActive(section.id)}
      ...
      <div className="relative min-w-0 flex-1 md:hidden">
        ...
        {menuOpen ? (
          ...
          >
            {ADVENTURE_SECTIONS.map((section, index) => {
              const isSelected = isSectionActive(section.id);
