...
export function CampaignRecentActivity({ activity, embedded = false }: CampaignRecentActivityProps) {
  const activityBody = (
    <>
      {activity.items.length === 0 ? (
        <p className={`${TYPE_PROSE_CLASS} ${embedded ? '' : 'mt-3'} text-sm text-prose-muted`}>
          {activity.emptyMessage}
        </p>
      ) : (
        <ul className={`${embedded ? '' : 'mt-3'} space-y-2`}>
  ...
  if (embedded) return activityBody;
