...
exports.HAVEN_SIMULATION_AXES = exports.HAVEN_SIMULATION_VERSION = void 0;
exports.defaultHavenSimulationAxes = defaultHavenSimulationAxes;
exports.emptyHavenSimulationState = emptyHavenSimulationState;
exports.parseHavenSimulationFromHints = parseHavenSimulationFromHints;
exports.mergeHavenSimulationIntoHints = mergeHavenSimulationIntoHints;
exports.mergeHavenSimulationPatch = mergeHavenSimulationPatch;
exports.bandIndexForValue = bandIndexForValue;
exports.bandMidpointForIndex = bandMidpointForIndex;
exports.formatHavenAxisBand = formatHavenAxisBand;
exports.isAxisEligibleForAdvance = isAxisEligibleForAdvance;
exports.buildBandCrossingSummary = buildBandCrossingSummary;
exports.deriveAxisDriversForPresentation = deriveAxisDriversForPresentation;
exports.advanceHavenSimulation = advanceHavenSimulation;
exports.applySimulationDeltas = applySimulationDeltas;
exports.buildDominantPressureHeadline = buildDominantPressureHeadline;
exports.buildHavenSimulationSnapshot = buildHavenSimulationSnapshot;
exports.formatHavenSimulationAxisLabel = formatHavenSimulationAxisLabel;
exports.listHavenSimulationBandLabels = listHavenSimulationBandLabels;
exports.axisValueForBandIndex = axisValueForBandIndex;
exports.HAVEN_SIMULATION_VERSION = 'haven-simulation-v1';
exports.HAVEN_SIMULATION_AXES = [
    'prosperity',
    'danger',
    'morale',
    'notoriety',
...
function defaultHavenSimulationAxes() {
    return {
...
function emptyHavenSimulationState() {
    return {
        version: exports.HAVEN_SIMULATION_VERSION,
        enabled: false,
        pausedReason: null,
...
function normalizeLockedAxes(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return {};
    const record = raw;
    const locked = {};
    for (const axis of exports.HAVEN_SIMULATION_AXES) {
        if (record[axis] === true)
            locked[axis] = true;
    }
    return locked;
}
function normalizeAxisValue(raw, fallback) {
...
function parseHavenSimulationFromHints(simulationHints) {
    const raw = simulationHints.havenSimulation;
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
        return emptyHavenSimulationState();
    }
    const record = raw;
    const pausedReason = typeof record.pausedReason === 'string' && record.pausedReason.trim()
    ...
    return {
        ...
        lastSimulatedAtEpochMinute: typeof record.lastSimulatedAtEpochMinute === 'string'
            ? record.lastSimulatedAtEpochMinute
            : null,
    };
}
function mergeHavenSimulationIntoHints(simulationHints, simulation) {
    return {
        ...simulationHints,
        havenSimulation: {
...
function mergeHavenSimulationPatch(simulationHints, patch) {
    const current = parseHavenSimulationFromHints(simulationHints);
    const next = {
        ...current,
        enabled: patch.enabled ?? current.enabled,
        pausedReason: patch.pausedReason !== undefined ? patch.pausedReason : current.pausedReason,
...
function bandMidpointForIndex(index) {
    return BAND_MIDPOINTS[Math.max(0, Math.min(4, index))] ?? NEUTRAL_AXIS_VALUE;
}
function formatHavenAxisBand(axis, value) {
...
function isAxisEligibleForAdvance(axis, elapsedMinutes) {
...
function axisDistanceFromNeutral(value) {
...
function crossingSeed(axis, fromIndex, toIndex, theme) {
    const themeSeed = theme ? theme.charCodeAt(0) : 0;
    return axis.charCodeAt(0) + fromIndex * 7 + toIndex * 13 + themeSeed;
}
function buildBandCrossingSummary(axis, fromIndex, toIndex, primaryTheme) {
...
function crossingTone(axis, toIndex, rising) {
    if (axis === 'danger' || axis === 'notoriety') {
        if (rising && toIndex >= 3)
            return 'escalation';
        if (rising)
    ...
    if (!rising && toIndex <= 1)
        return toIndex === 0 ? 'escalation' : 'warning';
    return 'neutral';
}
function computeAxisDrift(axis, axes, scale, context, drivers) {
    if (scale <= 0)
        return 0;
    ...
    delta += pullToNeutral;
    const addDriver = (targetAxis, reason, amount) => {
        if (targetAxis !== axis)
            return;
        delta += amount;
...
function deriveAxisDriversForPresentation(axes, context) {
    const drivers = {};
    const collect = (targetAxis, reason) => {
...
function advanceHavenSimulation(input) {
    const { simulation, elapsedMinutes, advanceMagnitude, nextEpochMinute, context } = input;
    ...
    const bandCrossings = [];
    for (const axis of exports.HAVEN_SIMULATION_AXES) {
        if (simulation.lockedAxes[axis])
            continue;
        if (!isAxisEligibleForAdvance(axis, elapsedMinutes))
            continue;
        ...
        if (afterIndex !== beforeIndex) {
            ...
            bandCrossings.push({
                ...
                fromBand: beforeBand,
                toBand: afterBand,
                summary: buildBandCrossingSummary(axis, beforeIndex, afterIndex, context.primaryTheme),
...
function applySimulationDeltas(simulation, deltas) {
    const nextAxes = { ...simulation.axes };
    for (const axis of exports.HAVEN_SIMULATION_AXES) {
        const delta = deltas[axis];
        if (delta === undefined || simulation.lockedAxes[axis])
            continue;
        nextAxes[axis] = clampAxisValue(nextAxes[axis] + delta);
    }
    return { ...simulation, axes: nextAxes };
}
function buildDominantPressureHeadline(simulation, axisDrivers) {
    ...
    for (const axis of exports.HAVEN_SIMULATION_AXES) {
        ...
        let score = distance;
        if (band.tone === 'escalation')
            score += 30;
        else if (band.tone === 'warning')
            score += 15;
        if (axis === 'danger' || axis === 'notoriety') {
            if (value > NEUTRAL_AXIS_VALUE)
                score += 10;
        }
...
function buildHavenSimulationSnapshot(simulationHints, axisDrivers = {}) {
    const simulation = parseHavenSimulationFromHints(simulationHints);
    const axes = exports.HAVEN_SIMULATION_AXES.map((axis) => {
...
function formatHavenSimulationAxisLabel(axis) {
    return AXIS_LABELS[axis];
}
function listHavenSimulationBandLabels(axis) {
    return AXIS_BANDS[axis];
}
function axisValueForBandIndex(index) {
    return bandMidpointForIndex(index);
}
//# sourceMappingURL=havenSimulation.js.map