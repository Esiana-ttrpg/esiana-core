...
exports.elapsedDaysBetweenEpochMinutes = elapsedDaysBetweenEpochMinutes;
exports.formatDowntimeElapsedLabel = formatDowntimeElapsedLabel;
exports.formatDowntimePeriodTitle = formatDowntimePeriodTitle;
exports.formatDowntimePeriodRollupHeadline = formatDowntimePeriodRollupHeadline;
exports.deriveDowntimePeriodGapsFromSessionEpochs = deriveDowntimePeriodGapsFromSessionEpochs;
const MINUTES_PER_DAY = 1440n;
function elapsedDaysBetweenEpochMinutes(startEpochMinute, endEpochMinute) {
    try {
...
function formatDowntimeElapsedLabel(days) {
    if (days <= 0n)
...
function formatDowntimePeriodTitle(input) {
    const promoted = input.promotedLabel?.trim();
    if (promoted)
        return promoted;
    const days = elapsedDaysBetweenEpochMinutes(input.startEpochMinute, input.endEpochMinute);
...
function deriveDowntimePeriodGapsFromSessionEpochs(input) {
    const current = input.currentEpochMinute;
    const sessions = [...input.sessionEpochs].sort((a, b) => a.sequenceOrder - b.sequenceOrder);
    if (sessions.length === 0) {
        const start = input.fallbackStartEpochMinute ?? '0';
        if (BigInt(current) <= BigInt(start))
            return [];
        return [
            {
    ...
    for (let i = 0; i < sessions.length - 1; i += 1) {
        ...
        if (BigInt(after.epochMinute) <= BigInt(before.epochMinute))
            continue;
        gaps.push({
...
//# sourceMappingURL=downtimePeriodProjection.js.map