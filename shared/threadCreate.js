...
exports.THREAD_KIND_CREATE_COPY = void 0;
exports.buildThreadBodyMarkdown = buildThreadBodyMarkdown;
exports.suggestThreadKindFromContext = suggestThreadKindFromContext;
exports.THREAD_KIND_CREATE_COPY = {
    mystery: {
        label: 'Mystery',
...
function buildThreadBodyMarkdown(kind) {
    switch (kind) {
        case 'mystery':
...
function suggestThreadKindFromContext(entityCategoryKey) {
    const key = (entityCategoryKey ?? '').trim().toLowerCase();
    if (key === 'quest' || key === 'quests')
        return 'foreshadowing';
    if (key === 'character' || key === 'characters' || key === 'npc')
        return 'mystery';
