...
export function buildLocationMetadataPatch(
  input: Record<string, unknown>,
): Partial<LocationMetadataFields> {
  const patch: Partial<LocationMetadataFields> = {};
  for (const key of LOCATION_METADATA_KEYS) {
    if (key in input) {
      (patch as Record<string, unknown>)[key] =
        parseLocationMetadata({ [key]: input[key] })[key];
    }
  }
  return patch;
}
