...
function buildFixtureGraph(): Map<string, WikiPageGraphNode> {
  const pages: WikiPageGraphNode[] = [
    ...
    node({ id: 'region', title: 'Sword Coast', parentId: 'locations', templateType: 'LOCATION' }),
    node({ id: 'town', title: 'Greenest', parentId: 'region', templateType: 'LOCATION' }),
    node({ id: 'tavern', title: 'Purple Dragon Inn', parentId: 'town', templateType: 'LOCATION' }),
    node({ id: 'room', title: 'Back Room', parentId: 'tavern', templateType: 'LOCATION' }),
    node({
      id: 'innkeeper',
      title: 'Innkeeper',
      parentId: 'tavern',
