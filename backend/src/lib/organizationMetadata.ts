...
export function buildOrganizationMetadataPatch(
  input: Record<string, unknown>,
): Partial<OrganizationMetadataFields> {
  const patch: Partial<OrganizationMetadataFields> = {};
  const parsed = parseOrganizationMetadata(input);
  for (const key of ORG_METADATA_KEYS) {
    if (key in input) {
