...
function normalizeSociety(raw: unknown): AncestrySociety | null {
  ...
  if (!name) return null;
  const id =
    typeof record.id === 'string' && record.id.trim() ? record.id.trim() : randomUUID();
  return {
    id,
    name,
...
export function validateAncestryParentChain(
  pageId: string,
  parentAncestryId: string | null,
  resolveMetadata: (id: string) => unknown,
): string | null {
  if (!parentAncestryId) return null;
  if (parentAncestryId === pageId) {
    return 'An ancestry page cannot be its own parent.';
  }
  const visited = new Set<string>([pageId]);
  let current: string | null = parentAncestryId;
  while (current) {
    if (visited.has(current)) {
      return 'Ancestry parent chain would create a cycle.';
    }
    visited.add(current);
    const parentMeta = parseAncestryMetadata(resolveMetadata(current));
    current = parentMeta.parentAncestryId;
  }
  return null;
}
