import { readLegacyMetadataField, syncMetadataIndexFields } from './codexMetadataShared.js';
...
export function buildRuleResourceMetadataPatch(
  input: Record<string, unknown>,
): Partial<RuleResourceMetadataFields> {
  const patch: Partial<RuleResourceMetadataFields> = {};
  for (const key of RULE_RESOURCE_METADATA_KEYS) {
    if (key in input) {
      (patch as Record<string, unknown>)[key] =
        parseRuleResourceMetadata({ [key]: input[key] })[key];
    }
  }
  return patch;
}
