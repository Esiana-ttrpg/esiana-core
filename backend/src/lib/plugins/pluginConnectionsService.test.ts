...
import { redactConnection } from './pluginConnectionsService.js';
...
test('redacted connection metadata never includes encrypted or raw credentials', () => {
  const row = {
    ...
    authType: 'oauth2', status: 'connected', credentialEnc: 'encrypted-secret-payload', accountLabel: 'Library account',
