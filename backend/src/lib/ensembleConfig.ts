/**
 * Campaign ensemble / party cast screen configuration.
 */
...
export interface EnsembleConfig {
  ...
  /** Phase 2: DM-authored narrative tension notes */
  tensionNotes: string[];
  /** Phase 3: campaign index landing preference */
...
export function normalizeEnsembleConfig(raw: unknown): EnsembleConfig {
  ...
  return {
    ...
    bannerImageUrl: coerceAssetReferenceUrl(parsed.bannerImageUrl) ?? defaults.bannerImageUrl,
...
export function parseEnsembleConfigPayload(body: unknown): EnsembleConfig | null {
  if (!body || typeof body !== 'object') return null;
  return normalizeEnsembleConfig(body);
}
